...
async function main() { const newName = readlineSync.question("Enter the new name: "); const extractedFolder = await extractTar(newName); findAndReplaceFileNames(extractedFolder, oldName, newName); findAndReplaceFileContent(extractedFolder, oldName, newName); console.log('Replacement process completed.'); }
